...
La communication gestionnaire d'abonnements → abonné utilise Redis PubSub :
...
1. Ajoutez le champ de filtre à la spec OpenAPI (`openapi.yml`)
2. Transmettez le filtre dans le flux d'abonnement (app.py → command_listener.py → subscriber.py)
3. Appliquez le filtre dans la tâche de téléchargement (`wis2.py`)

Exemple : Ajout d'un filtre `max_file_size` :
