...
L'interface web est disponible sur `http://localhost:8080`. Elle offre un moyen visuel de découvrir les jeux de données WIS2 disponibles, parcourir la hiérarchie des rubriques et créer des abonnements sans utiliser directement l'API REST.
...
La vue Recherche dans le catalogue interroge les enregistrements récupérés des trois catalogues de découverte mondiaux WIS2 (CMA, DWD, ECCC), fusionnés en une liste dédupliquée unique.
...
|Observations de surface de tous les pays
...
Les abonnements peuvent également être gérés de manière programmatique via l'API REST sur `http://localhost:5002`.

TIP: Utilisez l'interface Swagger sur `http://localhost:5002/swagger` pour l'exploration interactive de l'API.
...
NOTE: Le caractère `#` doit être encodé en URL sous la forme `%23` (sinon interprété comme fragment d'URL). Les barres obliques et `+` ne nécessitent pas d'encodage.
...
Les fichiers sont organisés par date sous votre répertoire cible :
...
Accédez à Grafana sur `http://localhost:3000` pour une surveillance visuelle.

NOTE: Les identifiants par défaut sont admin/admin. Vous devriez changer le mot de passe après la première connexion.
