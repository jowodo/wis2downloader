...
La comunicación gestor de suscripciones → suscriptor usa Redis PubSub:
...
1. Agregue el campo de filtro a la especificación OpenAPI (`openapi.yml`)
2. Transmita el filtro a través del flujo de suscripción (app.py → command_listener.py → subscriber.py)
3. Aplique el filtro en la tarea de descarga (`wis2.py`)

Ejemplo: Agregar un filtro `max_file_size`:
