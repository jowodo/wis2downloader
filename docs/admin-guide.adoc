...
* Using external managed Redis (AWS ElastiCache, Azure Cache for Redis)
* Running Redis with persistence enabled (AOF + RDB)
...
docker exec redis redis-cli DEL global:all_subscriptions

# Clear deduplication cache (allows re-download of all data)
docker exec redis redis-cli KEYS "wis2:notifications:*" | xargs -r docker exec -i redis redis-cli DEL
