...
Subscription Manager 与 Subscriber 之间的通信使用 Redis PubSub：
...
1. 将过滤器字段添加到 OpenAPI 规范（`openapi.yml`）
2. 通过订阅流程传递过滤器（app.py → command_listener.py → subscriber.py）
3. 在下载任务（`wis2.py`）中应用过滤器

示例：添加 `max_file_size` 过滤器：
