...
Web 界面可通过 `http://localhost:8080` 访问。它提供了一种可视化方式来发现可用的 WIS2 数据集、浏览主题层次结构，并无需直接使用 REST API 即可创建订阅。
...
目录搜索视图查询从三个 WIS2 全球发现目录（CMA、DWD、ECCC）获取的记录，合并为单个去重列表。
...
树形搜索视图根据所有已加载的 GDC 记录呈现完整的 WIS2 主题层次结构。展开节点可向下导航到各个主题。在顶部的过滤框中输入内容可搜索所有节点标签。点击叶节点可打开订阅侧边栏。
...
订阅也可以使用 `http://localhost:5002` 的 REST API 通过编程方式管理。

TIP: 使用 `http://localhost:5002/swagger` 的 Swagger UI 进行交互式 API 探索。
...
NOTE: `#` 字符必须 URL 编码为 `%23`（否则解释为 URL 片段）。斜杠和 `+` 不需要编码。
...
访问 `http://localhost:3000` 的 Grafana 进行可视化监控。

NOTE: 默认凭据为 admin/admin。首次登录后应更改密码。
