...
تتوفر واجهة المستخدم على العنوان `http://localhost:8080`. توفر طريقة مرئية لاكتشاف مجموعات بيانات WIS2 المتاحة، واستعراض التسلسل الهرمي للموضوعات، وإنشاء الاشتراكات دون استخدام REST API مباشرةً.
...
تستعلم طريقة عرض بحث الكتالوج عن السجلات المُجلَبة من كتالوجات اكتشاف البيانات العالمية الثلاثة لـ WIS2 (CMA وDWD وECCC)، مُدمَجةً في قائمة موحدة خالية من التكرار.
...
تُعرض في طريقة عرض بحث الشجرة التسلسل الهرمي الكامل لموضوعات WIS2 المُشتَقة من جميع سجلات GDC المُحمَّلة. وسِّع العقد للتنقل وصولًا إلى الموضوعات الفردية. اكتب في مربع الفلتر أعلى الصفحة للبحث عبر جميع تسميات العقد. انقر على عقدة نهائية لفتح الشريط الجانبي للاشتراك.
...
يمكن أيضًا إدارة الاشتراكات برمجيًا باستخدام REST API على `http://localhost:5002`.

TIP: استخدم واجهة Swagger على `http://localhost:5002/swagger` للاستكشاف التفاعلي لـ API.
...
NOTE: يجب ترميز الحرف `#` بـ URL كـ `%23` (وإلا يُفسَّر كجزء من URL). لا تتطلب الشُّرَط المائلة والـ`+` أي ترميز.
...
تُنظَّم الملفات حسب التاريخ تحت مجلدك المستهدف:
...
الوصول إلى Grafana على `http://localhost:3000` للمراقبة المرئية.

NOTE: بيانات الاعتماد الافتراضية هي admin/admin. يجب تغيير كلمة المرور بعد تسجيل الدخول الأول.
