...
يستخدم التواصل بين Subscription Manager وSubscriber نظام Redis PubSub:
...
1. إضافة حقل المرشح إلى مواصفة OpenAPI (`openapi.yml`)
2. تمرير المرشح عبر تدفق الاشتراك (app.py → command_listener.py → subscriber.py)
3. تطبيق المرشح في مهمة التنزيل (`wis2.py`)

مثال: إضافة مرشح `max_file_size`:
