...
Веб-интерфейс доступен по адресу `http://localhost:8080`. Он предоставляет визуальный способ обнаружения доступных наборов данных WIS2, навигации по иерархии тем и создания подписок без прямого использования REST API.
...
Представление «Поиск по каталогу» запрашивает записи, полученные из трёх глобальных каталогов обнаружения WIS2 (CMA, DWD, ECCC), объединённых в единый дедуплицированный список.
...
Представление «Поиск по дереву» отображает полную иерархию тем WIS2, полученную из всех загруженных записей GDC. Разворачивайте узлы для навигации до отдельных тем. Введите текст в поле фильтра вверху для поиска по всем меткам узлов. Нажмите на конечный узел для открытия боковой панели подписки.

NOTE: В представлении «Дерево» одновременно может быть активна только одна тема.

=== Создание подписки через интерфейс

Когда выбрана тема (из поиска по каталогу или представления «Дерево»), боковая панель подписки открывается справа.
...
|Приземные наблюдения из всех стран
...
Подписками также можно управлять программно с помощью REST API по адресу `http://localhost:5002`.

TIP: Используйте Swagger UI по адресу `http://localhost:5002/swagger` для интерактивного изучения API.
...
NOTE: Символ `#` должен быть URL-закодирован как `%23` (иначе интерпретируется как фрагмент URL). Слэши и `+` не требуют кодирования.
...
Файлы организованы по дате в вашем целевом каталоге:
...
Откройте Grafana по адресу `http://localhost:3000` для визуального мониторинга.

NOTE: Учётные данные по умолчанию: admin/admin. Рекомендуется изменить пароль после первого входа.
